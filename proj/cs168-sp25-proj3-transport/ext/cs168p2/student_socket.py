...
class StudentUSocket(StudentUSocketBase):
    ...
    * Send pending ACKs and FINs
    ...
    while not self.rx_queue.empty():
      ...
      if seq |LE| self.rcv.nxt:
        ...
        self.handle_accepted_seg(packet.tcp, packet.app[self.rcv.nxt |MINUS| packet.tcp.seq:])
      else:
        self.set_pending_ack()
        break
    ## End of Stage 3.2 ##
  ...
  def maybe_send(self):
    ...
    ## Start of Stage 4.3 ##
    remaining = snd.wnd |PLUS| snd.una |MINUS| snd.nxt
    while remaining > 0 and self.tx_data:
      send_len = min(self.mss, remaining, len(self.tx_data))
      # print(send_len)
      payload = self.tx_data[0: send_len]
      num_pkts += 1
